...
export function samplePlayers(): Player[] {
  ...
  const tops: Record<Position, number> = {
    WR: 345,
    RB: 325,
    QB: 355,
    TE: 245,
  ...
  const drops: Record<Position, number> = {
    WR: 4.2,
    RB: 4.8,
    QB: 6,
    TE: 5.5,
  ...
  for (const pos of Object.keys(sizes) as Position[]) {
    for (let i = 0; i < sizes[pos]; i++) {
      const projection =
        Math.round((tops[pos] - drops[pos] * i - Math.floor(i / 6) * 9) * 10) /
        10;
      rows.push({
        id: `demo-${pos.toLowerCase()}-${i + 1}`,
        name: names[pos][i] ?? `Demo ${pos} ${String(i + 1).padStart(2, "0")}`,
        team: "DEMO",
        position: pos,
        projection,
        positionalRank: i + 1,
        ...
        risk: (i % 5) / 10,
        upside: (i % 4) / 6,
        source: "Synthetic demonstration data — not current rankings",
        season: 2026,
        sample: true,
      });
    }
  }
  const offset: Record<Position, number> = {
    RB: 0,
    WR: 0,
    QB: 115,
    TE: 45,
  ...
  rows.sort(
    (a, b) =>
      b.projection - offset[b.position] - (a.projection - offset[a.position]),
  );
